...
The reference automation performs the structured part as one integrated sweep,
not as four independent layer jobs. It freezes bounded committed Daily together
with relevant current XLTM/LTM/MTM/STM bodies, applies the explicit layer and
split policy, returns all required changes and source dispositions, and closes
the resulting content with one validation, audit, index rebuild, and receipt.
...
flowchart LR
    ...
    F --> X["01 XLTM forest"] --> L["02 LTM tree"] --> M["03 MTM branch"] --> S["04 STM leaf"]
...
2. Which tree and branch own this leaf?
...
- XLTM owns the forest and its long-horizon map.
- LTM owns trees.
...
Inside an integrated sweep, new children are materialized only after their
required parents. A leaf should not appear before its branch and tree owners
exist. This is a structural transaction order, not a parent-first semantic
workflow. The model or reviewer first decides the whole required forest delta.
This makes invalid structure detectable without an AI model.
...
- knowledge remains attached to the evidence and tree that own it
...
- projects can retain their tree, branch, and leaf context instead of becoming isolated summaries
